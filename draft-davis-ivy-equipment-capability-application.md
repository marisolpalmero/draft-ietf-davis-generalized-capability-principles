...
### Availability Independent of Deployment
...
### Relationship to the RFC 9196 Capability Advertisement Mechanism

{{RFC9196}} defines `ietf-system-capabilities`, a standard YANG container with an open `per-node-capabilities` augmentation point and a dual-mode delivery model: the same schema is used whether capability data is queried from a live device at runtime via NETCONF {{RFC6241}} or RESTCONF {{RFC8040}}, or consumed from a vendor-supplied file before deployment via YANG instance data {{RFC9195}}.

{{RFC9196}} defines **how** to advertise capabilities: the container, the query path, and the delivery format.  It is domain-agnostic and carries no definition of what a capability is, how capabilities compose hierarchically, how entitlements constrain their use, or how functional dependencies relate them.

This framework defines **what** flows through that mechanism: the semantic language, composition rules, entitlement model, and lifecycle scope of equipment capability specifications.  The two are complementary and non-overlapping:

*  This framework is the semantic language and composition rules: the content.

*  {{RFC9196}} is the delivery container and query mechanism: the pipe.

*  {{RFC9195}} is the pre-deployment file delivery format: the envelop.

This framework is intentionally independent of the delivery mechanism.  The same capability specification can be queried from a live device or shipped as a vendor-supplied file without any change to its content or structure.
